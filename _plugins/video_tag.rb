...
module Jekyll
  ...
  class VideoTag < Liquid::Tag
    ...
    def render(context)
      ...
      if @videos.size > 0
        inline_video =  "<video #{sizes} preload='metadata' autoplay loop muted loading=\"lazy\" #{poster}>"
        @videos.each do |v|
          inline_video << "<source src='#{v}' #{types[File.extname(v)]}>"
        end
        inline_video += "</video>"
      else
...
Liquid::Template.register_tag('inline_video', Jekyll::VideoTag)
